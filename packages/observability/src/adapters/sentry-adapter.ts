...
export class SentryAdapter implements ILogger, ITracingProvider {
  ...
  createSpan(name: string, attributes?: Record<string, any>): ISpan {
    ...
    const span = transaction?.startChild({ op: name, data: attributes });
  ...
  async withSpan<T>(
    ...
    const transaction = Sentry.startTransaction({ name, data: attributes });
    Sentry.getCurrentHub().configureScope(scope => scope.setSpan(transaction));

    const span = new SentrySpan(transaction);
  ...
  getTraceContext(): string | undefined {
    const transaction = Sentry.getCurrentHub().getScope()?.getTransaction();
    return transaction ? `${transaction.traceId}-${transaction.spanId}` : undefined;
  }
}
